...
(function(){
  /* resolve page links from pretty-URL folders back to root */
  var AI_BASE=(function(){
    var folders=['content-creator-portfolio','editor-portfolio','ai-film-making-portfolio','ai-kfc','ai-awareness'];
    var segs=location.pathname.split('/').filter(Boolean);
    for(var i=0;i<segs.length;i++){if(folders.indexOf(segs[i])>-1)return '../';}
    return '';
  })();
  var V = [
    ...
    {id:'YjHGi1JDjNE', t:'Hatta — A Reset from City Life', r:['Idea','Concept','Video Editing','Videography'], tag:'Social', k:'Short', cat:['vid','edt']},
    {id:'lnsdQu2q1jM', t:'Abra — The Best AED 1 You&rsquo;ll Ever Spend', r:['Idea','Concept','Video Editing','Videography'], tag:'Social', k:'Short', cat:['vid','edt']},
  ...
  var H = [
    {id:'Jl8S3iZ8elU', t:'Showreel 2026', r:['Videography','Video Editing','Colour Grading'], tag:'Showreel', m:'Selected Work — 2026', k:'Reel', cat:['cin','vid','edt','grd']},
  ...
  var VE_V = [
    ...
    {id:'YjHGi1JDjNE', t:'Hatta — A Reset from City Life', r:['Idea','Concept','Video Editing'], k:'Short'},
    {id:'lnsdQu2q1jM', t:'Abra — The Best AED 1 You&rsquo;ll Ever Spend', r:['Idea','Concept','Video Editing'], k:'Short'},
  ...
  var VE_H = [
    {id:'Jl8S3iZ8elU', t:'Showreel 2026', r:['Video Editing'], m:'Selected Work — 2026', k:'Reel'},
  ...
  function tile(p, n){
    ...
    return '<a class="tile rv" data-vid="'+p.id+'" data-title="'+p.t+'" data-tag="'+(p.tag||p.m||'')+'" data-cats="'+(p.cat||[]).join(' ')+'" '+
      ...
      '<div class="tile__media">'+imgTag(p.id,p.t+' — still',p.o==='v'||p.o==='s')+
  ...
  // AI Film Making page (own order; per-tile orientation)
  var AI = [
    {id:'viFbYgPKV-0', t:'KFC — Independent Spec Campaign', r:['Creative Strategy','Brand Strategy','Concept Development','Creative Direction','Film Direction','Prompt Engineering','AI Continuity Direction','AI-First Creative Workflow'], k:'AI Film', o:'h', page:'ai-kfc.html'},
    {id:'DOWMFbRUB1o', t:'AI Awareness Short Film', r:['Creative Strategy','Concept Development','Creative Direction','Film Direction','Prompt Engineering','AI Continuity Direction','Post Production'], k:'AI Film', o:'h', page:'ai-awareness.html'},
    {id:'AQ7GorfyO4g', t:'Ajman Freezone', r:['End to End Campaign Design'], k:'AI Film', o:'s'}
  ];
  window.renderAI=function(sel){
    var el=document.querySelector(sel);if(!el)return;buildPlayer();
    el.className='grid grid--ai';
    el.innerHTML=AI.map(function(p,i){
      var html=tile(p,i+1).replace('class="tile rv"','class="tile rv tile--'+(p.o||'h')+'"');
      if(p.page){
        html=html.replace('href="https://youtu.be/'+p.id+'"','href="'+AI_BASE+p.page+'"')
                 .replace('data-vid="'+p.id+'"','data-page="'+p.id+'"');
      }
      return html;
    }).join('');
    bindTiles(el);initReveals(el);
  };
