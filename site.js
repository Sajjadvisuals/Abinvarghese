...
(function(){
  ...
  /* Resolve internal links from any depth: pretty-URL folders (e.g. /ai-kfc/) need to climb to root */
  var BASE=(function(){
  ...
  function R(p){return /^(https?:|mailto:|tel:|#)/.test(p)?p:BASE+p;}

  /* ---- NAV ---- */
  function navHTML(){
    function link(href,label,id){
      return '<a href="'+R(href)+'"'+(PAGE===id?' class="active"':'')+'><span class="plus">+</span>'+label+'</a>';
    }
    var menuActive=(PAGE==='about'||PAGE==='contact')?' is-active':'';
    return '<a class="nav__brand" href="'+R('index.html')+'"><span>Abin Varghese</span><span>Content Creator / Video Editor / Videographer</span></a>'+
  ...
  function overlayHTML(){
    function biglink(href,label,id,num,work){
      ...
      return '<a href="'+R(href)+'"'+(cls.length?' class="'+cls.join(' ')+'"':'')+'><span class="navmenu__i">'+num+'</span>'+label+'<span class="navmenu__ar">↗</span></a>';
    }
    return '<div class="navmenu__bg" data-mclose></div>'+
      '<div class="navmenu__panel">'+
        ...
        '<nav class="navmenu__links">'+
          ...
          biglink('contact.html','Contact','contact','06')+
        '</nav>'+
